...
def build_model_input(filename, batch_size, num_epochs, parquet=False):
    def _build_model_input_from_csv(filename, batch_size, num_epochs):
        def parse_csv(value):
    ...
    def _build_model_input_from_parquet(filename, batch_size, num_epochs):
        from tensorflow.python.data.experimental.ops import parquet_dataset_ops

        def split_lable(value):
            labels = value.pop(LABEL_COLUMN[0])
            features = value
            return features, labels

        columns = {}
        columns.update({'I' + str(i): tf.int64
                        for i in range(1, 14)})  # 1-13 inclusive
        columns.update({'C' + str(i): tf.string
                        for i in range(1, 27)})  # 1-26 inclusive
        columns.update({'clicked': tf.int64})

        dataset = parquet_dataset_ops.ParquetDataset(filename, columns)
        dataset = dataset.shuffle(buffer_size=20000,
                                  seed=2021)  # fix seed for reproducing
        dataset = dataset.repeat(num_epochs)
        dataset = dataset.prefetch(batch_size)
        dataset = dataset.batch(batch_size)
        dataset = dataset.map(split_lable, num_parallel_calls=28)
        dataset = dataset.prefetch(1)
        return dataset

    if parquet:
        return _build_model_input_from_parquet(filename, batch_size,
                                               num_epochs)
    else:
        return _build_model_input_from_csv(filename, batch_size, num_epochs)
...
def train(sess_config,
    ...
    stop_hook = tf.train.StopAtStepHook(last_step=steps - 1)
    summary_hook = tf.train.SummarySaverHook(save_steps=args.save_steps,
    ...
    log_hook = tf.train.LoggingTensorHook(
        ...
        every_n_iter=100,
        at_end=True)
    hooks = [stop_hook, log_hook]
    # TODO: After adding summary hook, summary_op will be run on each iteration which leads a drop on perf.
...
def eval(sess_config, model, data_init_op, steps, checkpoint_dir):
...
def main(tf_config=None, server=None):
    ...
    if args.parquet and not args.tf:
    ...
    # TODO: read rows of parquet.
    # no_of_training_examples = sum(1 for line in open(train_file))
    # no_of_test_examples = sum(1 for line in open(test_file))
    no_of_training_examples = 8000000
    no_of_test_examples = 2000000
    ...
    train_dataset = build_model_input(train_file, batch_size, no_of_epochs,
                                      args.parquet and not args.tf)
    test_dataset = build_model_input(test_file, batch_size, 1, args.parquet
                                     and not args.tf)
    ...
    train(sess_config, model, train_init_op, train_steps, checkpoint_dir,
          tf_config, server)
    if not (args.no_eval or tf_config):
        eval(sess_config, model, test_init_op, test_steps, checkpoint_dir)
...
def get_arg_parser():
    ...
    parser.add_argument('--parquet',
                        help='Use parquet format dataset, only for Deeprec.',
                        action='store_true')
    return parser
