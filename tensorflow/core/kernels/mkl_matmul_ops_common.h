...
class MklDnnMatMulBwdFilterPrimitive : public MklPrimitive {
 ...
 private:
  ...
  struct MklDnnMatMulBwdContext {
    // MKL-DNN memory.
    std::shared_ptr<dnnl::memory> src_mem;
    std::shared_ptr<dnnl::memory> diff_weight_mem;
    ...
    std::shared_ptr<dnnl::inner_product_backward_weights::desc> bwd_desc;
    std::shared_ptr<dnnl::inner_product_backward_weights::primitive_desc>
        bwd_pd;
    ...
    // output
    std::shared_ptr<dnnl::memory::desc> diff_weight_md;
    std::shared_ptr<dnnl::memory::desc> diff_bias_md;

    // Inner-product primitive.
    std::shared_ptr<dnnl::primitive> matmul_bwd;
